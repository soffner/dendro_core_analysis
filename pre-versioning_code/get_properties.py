...
def load_data(file, unit_base):
    ...
    del f
    return den, x, m, h, u, b, v, fmol, fneu, partlist, partmasses, partids

def calc_nh2(den, fmol, fneu, unit_base, helium_mass_fraction=0.284, mh=1.67e-24):
    ## Calculate n_H2
    return unit_base['UnitMass_in_g']/ unit_base['UnitLength_in_cm']**3* den*fmol*fneu*(1-helium_mass_fraction)/(2.0*mh)
    
def get_leaf_properties(dendro, den, x, m, h, u, b, v, snapshot_no, partlist, partmasses, partids):
    """ dendro - dendrogram
    ...
    for leaf in dendro:
        if leaf.is_leaf:
            ...
            leaf_id.append(str(snapshot_no)+str(leaf.idx))
    ...
    # leaf_centidx
    return leaf_masses, leaf_maxden, leaf_centidx, leaf_centpos, leaf_vdisp, leaf_vbulk, leaf_evals, leaf_evecs, leaf_halfmass, leaf_reff, leaf_bmean, leaf_mage, leaf_ke, leaf_grave, leaf_sink, leaf_sinkallm, leaf_sinkallid, leaf_id, leaf_cs
    
def load_dendrogram(dendro_file, nh2):
    ...
    if os.path.isfile(dendro_file):
        ...
        start = time.time()
        d = Dendrogram.compute(nh2, min_value=1e4, min_delta=1e4, min_npix=100) 
        end = time.time()
        
        print("Done computing dendogram .... Time to make dendrogram [min] is", (end-start)/60.0)
        Dendrogram.save(d, file=dendro_file)
    ...
    plt.savefig(dendro_file[0:7]+"_Dendrogram.png")
    
    return d


def calc_profiles(dendro, nh2, x, v, nbin, maxsize=0.5, plotleaf=False, saveall=False):
    """
...
def interpolate_profiles(nbin, veldisp, radii, leaf_cs):
    ...
    plt.savefig("Interpolated_Densities.png")
    ...
    ax.set_xscale('log')
    ax.set_xlim([0.001, 2e-1])
    plt.savefig("Interpolated_veldisp.png")
